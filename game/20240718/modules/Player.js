...
class Player {
  constructor(ctx, pos, speed) {
    ...
    this.isJumpButtonReleased = true;
  }
  ...
  update(buttons) {
    ...
    if (buttons.a.touchedId) {
      if (this.speed.y === 0 && !this.isJumping && this.isJumpButtonReleased) {
        this.speed.y = 0;
        this.isJumping = true;
        this.isJumpButtonReleased = false;
        if (this.isDashing) {
  ...
  checkFloor(floor) {
    // If the player is on the floor, stop the player.
